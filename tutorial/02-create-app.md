...
1. 将 **产品名称设置为** `GraphTutorial` ，将 **语言设置为** **Objective-C。**
1. 填写其余字段，然后选择"下 **一步"。**
1. 选择项目的位置，然后选择"创建 **"。**

## <a name="install-dependencies"></a>安装依赖项

在继续之前，请安装一些稍后将使用的附加依赖项。
...
在此部分中，你将为应用创建视图：登录页、选项卡栏导航器、欢迎页面和日历页。 你还将创建活动指示器覆盖层。

### <a name="create-sign-in-page"></a>创建登录页

1. 在 **Xcode 中展开 GraphTu一l** 文件夹，然后选择 **ViewController.m** 文件。
1. 在 **文件检查器** 中 **，将文件** 的名称更改为 `SignInViewController.m` 。

    ![文件检查器屏幕截图](./images/rename-file.png)

1. 打开 **SignInViewController.m，** 并将其内容替换为以下代码。
...
1. 选择 **ViewController.h** 文件。
1. 在 **文件检查器** 中 **，将文件** 的名称更改为 `SignInViewController.h` 。
1. 打开 **SignInViewController.h，** 将所有实例 `ViewController` 更改为 `SignInViewController` 。

1. 打开 **Main.storyboard** 文件。
1. 展开 **"视图控制器场景**"，然后选择 **"视图控制器"。**

    ![选中视图控制器的 Xcode 屏幕截图](./images/storyboard-select-view-controller.png)

1. 选择标识 **检查器**，然后将 **类下拉列表更改为** **SignInViewController。**
...
1. 选中按钮后 **，选择情节** 提要底部的"对齐"按钮。 在容器约束 **中** 同时选择"水平"和"垂直"，将其值保留为 0，然后选择"添加 **2 个约束"。**
...
1. 选择新的 **图像** 资源，并使用 **属性检查** 器将其 **名称** 设置为 `DefaultUserPhoto` 。
...
1. 打开 **WelcomeViewController.h，** 在声明中添加以下 `@interface` 代码。
...
1. 打开 **WelcomeViewController.m，** 并将其内容替换为以下代码。
...
1. 在 **编辑器菜单** 上，选择 **"解决自动布局** 问题"，然后选择"在欢迎视图控制器中所有视图下方添加缺少 **的约束"。**
...
1. 打开 **CalendarViewController.h，** 在声明中添加以下 `@interface` 代码。
...
1. 选择刚刚添加的文本视图。 在编辑器 **上**，选择 **"嵌入"，** 然后选择 **"滚动视图"。**
...
1. 打开 **SpinnerViewController.h，** 在声明中添加以下 `@interface` 代码。
...
1. 打开 **SpinnerViewController.m，** 并将其内容替换为以下代码。
